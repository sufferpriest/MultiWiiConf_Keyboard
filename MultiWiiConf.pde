...
float rightX = 0;
float rightY = 0;
float tolerance = 0.05;  // set tolerance value for joysticks
...
void setup() {
  ...
  try {
  gamepad = controll.getDevice("Comait CG2400-B");
  ...
  rightStick = new ControllStick(gamepad.getSlider(4), gamepad.getSlider(3));
  }catch(Exception e){}
  


/*controlP5.addTab("extra").setColorBackground(color(0, 160, 100)).setColorLabel(color(255)).setColorActive(color(255,128,0));
controlP5.getTab("default").activateEvent(true).setLabel("my default tab").setId(1);
controlP5.getTab("extra").activateEvent(true).setId(2); */
//controlWindow = controlP5.addControlWindow("controlP5window",100,100,400,400).hideCoordinates().setBackground(color(40));
...
LeftStick = controlP5.addSlider2D("Left")
         ...
         .setArrayValue(new float[] {600, 600})
         ;
...
void keyPressed() {
    if (key == CODED) {
       float rx = (float)RightStick.getArrayValue()[0] - 900;
       float ry = (float)RightStick.getArrayValue()[1] - 900;
      if (keyCode == UP) {
        RightStick.setArrayValue(new float[] {rx,  ry - MAX_STEP});
        println("Key pressed: Down arrow:" + rx + ", " + ry);
      } else if (keyCode == DOWN) {
        RightStick.setArrayValue(new float[] {rx,  ry + MAX_STEP});
      } else if (keyCode == LEFT) {
        RightStick.setArrayValue(new float[] {rx - MAX_STEP,  ry});
      } else if (keyCode == RIGHT) {
        RightStick.setArrayValue(new float[] {rx + MAX_STEP,  ry});
      }
    }else {
       float lx = (float)LeftStick.getArrayValue()[0] - 900;
       float ly = (float)LeftStick.getArrayValue()[1] - 900;
      if (key == 'w') {
        LeftStick.setArrayValue(new float[] {lx,  ly - MAX_STEP});
        println("Key pressed: w arrow:" + lx + ", " + ly);
      } else if (key == 's') {
        LeftStick.setArrayValue(new float[] {lx,  ly + MAX_STEP});
      } else if (key == 'a') {
        LeftStick.setArrayValue(new float[] {lx - MAX_STEP,  ly});
      } else if (key == 'd') {
        LeftStick.setArrayValue(new float[] {lx + MAX_STEP,  ly});
      }
    }
...
private ArrayList<Byte> requestMSP(int msp) {
  return  requestMSP( msp, null);
}

//send multiple msp without payload
private ArrayList<Byte> requestMSP (int[] msps) {
...
private ArrayList<Byte> requestMSP (int msp, Character[] payload) {
  if(msp < 0) {
   return null;
  }
  ArrayList<Byte> bf = new ArrayList<Byte>();
...
void sendRequestMSP(ArrayList<Byte> msp) {
...
if (leftStick != null) {
  ...
  leftX = -leftStick.getY();
  leftY = leftStick.getX();
  rightX = -rightStick.getY();
  rightY = rightStick.getX();

  if(leftY < -tolerance) {
    leftY = map(leftY, -1, -tolerance, 2000, 1500);
  }  
  else if(leftY > tolerance) {
    leftY = map(leftY, tolerance, 1, 1500, 1000);
  }  
  else {
    leftY = 1500;
  }
  ...
  if(leftX < -tolerance) {
    leftX = map(leftX, -1, -tolerance, 2000, 1500);
  }  
  else if(leftX > tolerance) {
       leftX = map(leftX, tolerance, 1, 1500, 1000);
  }  
  else {
        leftX = 1500;
  }
  ...
  if(rightY < -tolerance) {
     rightY = map(rightY, -1, -tolerance, 2000, 1500);
  }  
  else if(rightY > tolerance) {
     rightY = map(rightY, tolerance, 1, 1500, 1000);
  }  
  else {  
    rightY = 1500; 
  }
  ...
  if(rightX < -tolerance) {
    rightX = map(rightX, -1, -tolerance, 2000, 1500);
  }  
  else if(rightX > tolerance) {
     rightX = map(rightX, tolerance, 1, 1500, 1000);
  }  
  else {
    rightX = 1500; 
  }
...
RightStick.setArrayValue(new float[] {rightX-900,2100-rightY});
  ...
  //keyboard or mouse
  leftX = LeftStick.getArrayValue()[0];
  leftY = 2100.0 - LeftStick.getArrayValue()[1] + 900;
  rightX = RightStick.getArrayValue()[0];
  rightY = 2100.0 - RightStick.getArrayValue()[1] + 900;
}
